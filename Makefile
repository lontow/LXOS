...
kernel.bin: boot/kernel_entry.o init ${OBJ}
	i386-elf-ld -o $@ -T  kernel.ld  boot/kernel_entry.o ${OBJ} -b binary init  --oformat binary
echo:
	echo ${OBJ}
	echo ${C_SOURCES}

# 调试
kernel.elf: boot/kernel_entry.o  init ${OBJ}
	i386-elf-ld -T kernel.ld  -o $@  boot/kernel_entry.o ${OBJ} -b binary init    
...
boot/loader.bin: boot/loader.o boot/start.o  
	i386-elf-ld  -Ttext 0x7c00 -e boostart  -S   -o bootloader.o $^
